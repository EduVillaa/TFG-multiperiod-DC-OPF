...
def plot_generation_mix_pie(
    ...
    generation_data = {
        ...
        "Reservoir hydro discharge": safe_sum(dispatch_clean, "Hydro_discharge"),
    ...
    if include_PHS_discharge:
        generation_data["PHS discharge"] = safe_sum(dispatch_clean, "PHS_discharge")
...
def plot_generation_mix_pie1(
    dispatch_clean,
    include_storage_discharge=False,
    ...
    Usa las mismas categorías que el gráfico de generación real:
        PV, Wind, hydro, Biomass, Nuclear, CCGT

    En este caso no se incluye oil+lignite+waste.
    ...
    include_other : bool
        Si True, incluye la columna Other si existe.
    ...
    generation_data = {
        ...
        "Wind": safe_sum(dispatch_clean, "Wind"),
        "hydro": (
            safe_sum(dispatch_clean, "ror")
            + safe_sum(dispatch_clean, "Hidroelectric_discharge")
        ),
        "Biomass": safe_sum(dispatch_clean, "biomass"),
        "Nuclear": safe_sum(dispatch_clean, "Nuclear"),
        "CCGT": safe_sum(dispatch_clean, "CCGT"),
    ...
    if include_other:
        generation_data["Other"] = safe_sum(dispatch_clean, "Other")
